<?php
	...
	class Internet_model {
		...
		{
			$sql = "SELECT r.idRecibo, r.valorRecibo, r.consumo, r.fechaPago FROM recibo r WHERE r.estado = 0 AND r.idServicio = 4 ORDER BY r.fechaPago ASC";
		...
		{
			$sql = "SELECT r.idRecibo, r.valorRecibo, r.consumo, r.fechaPago FROM recibo r WHERE r.estado = 1 AND r.idServicio = 4 ORDER BY r.fechaPago ASC";
		...
		public function insertar_recibo($valorRecibo, $consumo, $fechaPago){
			...
			$sql = "INSERT INTO recibo (valorRecibo, idServicio, consumo, fechaPago, estado) VALUES ('$valorRecibo', 4, '$consumo', '$fechaPago', 0)";
			$resultado = $this->db->query($sql);
